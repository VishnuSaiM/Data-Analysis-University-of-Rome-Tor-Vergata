...
tmp_table=[fileCPALL(:,curr_freq_index-1), fileCPALL(:,curr_freq_index)];
table_timetable=table2timetable(tmp_table);
plot(table_timetable.VarName51,table_timetable{:,1},'k','LineWidth',1.5);
ylabel('[dBm]');
set(gca,'FontSize',16);
...
plot(tt_table_51.VarName51,tt_table_51{:,1},'LineWidth','k',1.5);
...
curr_gain=(9.73/(curr_lambda*10^(af_sel_frequencies(index_freq_af)/20)))^2;
curr_effective_area=curr_lambda^2*curr_gain/(4*pi);
tmp_table=[fileCPALL(:,curr_freq_index-1), varfun(@ (x) sqrt(377*10.^((x-30)/10)/curr_effective_area),fileCPW3(:,curr_freq_index))];
%tmp_table=[fileCPALL(:,curr_freq_index-1), varfun(@ (x) 1/sqrt(20)*10.^((x+af_sel_frequencies(index_freq_af))/20),fileCPW3(:,curr_freq_index))];
table_timetable=table2timetable(tmp_table);
plot(table_timetable.VarName13,table_timetable{:,1},'LineWidth',1.5);
...
sum_all_5G=(tt_table_all{:,24}).^2;
for index=24:27
    sum_all_5G=sum_all_5G+(tt_table_all{:,index}).^2;
end
sum_all_5G=sqrt(sum_all_5G);

tt_table_sum=[table_1(2:num_ts_1,1), array2table(sum_all_pre_5G), array2table(sum_all_5G), array2table(sum_all)];
...
peaks_5G=zeros(size(sum_all_5G,1),1);

cum_peaks_5G=zeros(size(sum_all_5G,1),1);
cum_peaks_5G_holidays=zeros(size(sum_all_5G,1),1);
cum_peaks_5G_workingdays=zeros(size(sum_all_5G,1),1);
...
for i=2:size(sum_all_5G,1)
    if sum_all_5G(i)-sum_all_5G(i-1) > peak_threshold                            
        peaks_5G(i)=1;
...
plot(tt_timetable_sum.VarName1,cum_peaks_5G_workingdays,'g--','LineWidth',1.5);
...
plot(tt_timetable_sum.VarName1,tt_timetable_sum.sum_all_2G,'g-','LineWidth',1.5);
plot(tt_timetable_sum.VarName1,tt_timetable_sum.sum_all_3G,'m-','LineWidth',1.5);
plot(tt_timetable_sum.VarName1,tt_timetable_sum.sum_all_pre5,'b-','LineWidth',1.5);
...
plot(tt_timetable_sum.VarName1,tt_timetable_sum.sum_all_2G,'g-','LineWidth',1.5);
plot(tt_timetable_sum.VarName1,tt_timetable_sum.sum_all_3G,'m-','LineWidth',1.5);
plot(tt_timetable_sum.VarName1,tt_timetable_sum.sum_all_4G,'b-','LineWidth',1.5);
plot(tt_timetable_sum.VarName1,tt_timetable_sum.sum_all_5G,'r-','LineWidth',1.5);
...
grid off
%set(gca, 'YScale', 'log')
xlim(datetime([2021 2022],[10 3],[23 4]));
ylim([0 0.52]);
...
plot(tt_table_51.Timestamp,tt_table_51{:,1},'k','LineWidth',1.5);
